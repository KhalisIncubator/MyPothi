...
import { ISchemaJSON, IConifg } from './database-interfaces';
...
export { $dbPath, $dbSchema, downloadProg };
...
const downloadDB = async () => {
  RNFetchBlob.config( {
    ...
    .fetch(
      'GET',
      'https://banidb.com/databases/sttmdesktop-evergreen.zip',
      {
        // some headers ..
      },
    )
...
const readSchema = (): Promise<ISchemaJSON> => RNFetchBlob.fs
  .readFile(
    `${dirs.DocumentDir}/sttmdesktop-evergreen/realm-schema-evergreen.json`,
    'utf8',
  )
  ...
  return new Promise( ( resolve, reject ) => {
    ...
    switch ( searchType ) {
      ...
      case consts.SEARCH_TYPES.FIRST_LETTERS_ANYWHERE: {
        ...
        let operator = searchType === consts.SEARCH_TYPES.FIRST_LETTERS
          ? 'BEGINSWITH'
          : 'CONTAINS';
