...
const fetchAllPothis = (): string[][] => {
  ...
  const pothis = localRealm.objects<storedPothi>( 'Pothi' );
  pothis.forEach( ( pothi ) => {
...
const getCurrentItems = ( currentPothi, pothiID ) => {
  ...
  for ( let i = 0; i < pothi.items.length; i++ ) {
    if ( pothi.items[i].isValid() ) {
      items.push( pothi.items[i] );
    }
  }
  return items;
};
 ...
 */
const removeFromPothi = ( currentPothi, itemId ) => {
