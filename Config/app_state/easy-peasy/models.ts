import { createStore, action, createContextStore, computed } from 'easy-peasy';

import { SearchModel, StoreModel, CurrentModel, GutkaModel, ViewerModel, EditModel } from './interfaces';
...
const currentModel: CurrentModel = {
  ...
  updateItems: action((state, payload) => {
    if (payload) {
      ...
      state.currentItems = getCurrentItems(state.currentName[0], state.currentName[1]);
    }
  }),
